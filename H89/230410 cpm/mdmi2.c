/**************************************************
/* Modem Include file 2 for HDOS and CP/M
...
*/
  ...
  do  {
      ...
      if( (first == SOH)||(first == STX) )
          ...
          if(( j == cnt)&& ((scur + scomp)  == 255))  /*<-- becomes this #*/
              {
              if( (scur == (0xff & (snum + 1))) )
                  {                    /* Good sector count */
                  switch( mode )
                    ...
                    case 0: chksum = 0;
                            for( j = 0; j < transize; j++)
                                {
                                *(rcvptr+j) = tbuff[j+2] ;  /*Readline(1);*/
                                chksum = (chksum + *(rcvptr+j)) & 0xff ;
                                }
                    ...
                    case 1: crc = 0 ;             /* CRC mode */
                            ...
                            chk1 = tbuff[j+2] ;  /*Readline( 1 ) ;     /* hi CRC byte */
                            chk2 = tbuff[j +3];  /*Readline( 1 ) ;     /* lo CRC byte */
                            if ( ((chk1 << 8) + chk2 ) == crc )
                                ok = TRUE ;
                            break ;
                    }
                  ...
                  if ( ok )
                      ...
                      if( ((unsigned) rcvptr - (unsigned) rcvbuf) >= BLOCK)
              ...
              {
              printf("\nError # %d - scur %x scomp %x + %x Rcv Err %d\n",
                      errors, scur, scomp, scur+scomp, err);
  ...
  if(( first == EOT )&&( errors < errmax ))
      ...
      printf("\nAborting");
  if( rcvptr > rcvbuf)    /* clear buffer */
      {
      while( (rcvptr - rcvbuf)%BLKSIZE != 0 )
          *rcvptr++ = 0;
      write( fp, rcvbuf, (rcvptr - rcvbuf));
      }
  fclose( fp );
...
SendFX( fp, filename )
  ...
  if(xyModem == 'Y')   /* Ymodem transfer, add file name */
      ...
      nbytes = read( fp, rcvbuf+128, BLOCK );  /* leave 128 bytes for name block */
      nbytes += 128;                     /* include fill in bytes read */
      rcvptr = rcvbuf;
      chptr = filename;
      while(*chptr !=0 )                 /* load filename */
          *rcvptr++ = *chptr++;
      while((rcvptr - rcvbuf) < 128)     /* zero fill first 128 bytes */
          *rcvptr++ = 0 ;
      transize = 128;
      snum = 0;
/*********
    ...
    ****  */
      rcvptr = rcvbuf;     /* Reset rcvbuf pointer to start */
      }
    else
      ...
      do  {                /* block send loop */
          ...
          if ( (first == ACK) ||(first == 'C') ) /* Successful block send,
              ...
              if(snum == 0)        /* sent filename, looking for C */
                  ...
                  rcvptr += 128 ;    /* update buffer pointer */
                  transize = tranSize;
                  }
                else
              ...
              bytesSent = (unsigned) rcvptr - (unsigned) rcvbuf;
              ...
              switch(first)
                  ...
                  case(CAN): printf("\nSender Cancel\n");
                             ...
                             goto ESCAPE;
                             break;
                  default:   printf("\nError! Garbled ACK, Sector # %d, Got %xH\n",
                                            snum, first ) ;
                  }
              }
...
#asm
        mvi     A,0
        lxi     B,083h                  ; line, echo, no word wrap
        SCALL   6                       ; .CONSL
#endasm
}
